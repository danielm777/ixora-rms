...
import com.ixora.rms.agents.websphere.v50.RelatedVersionBehaviour;
import com.ixora.rms.agents.websphere.v50.WebSphereEntity;
...
final class VersionBehaviour implements RelatedVersionBehaviour, PmiConstants {
    ...
    public boolean acceptPmiDataAsCounter(WebSphereEntity we, PmiDataInfo di) {
        int type = di.getType();
        if(type < TYPE_INT || type > TYPE_RANGE) {
            return false;
        }
        ...
        } else if(entityName.equals("webAppModule.servlets.urls")) {
        	return false;
        } else {
	...
	public boolean acceptPerfDescriptorAsEntity(PerfDescriptor pd) {
		...
		if(pdName.equals("cacheModule")
				|| pdName.equals("webAppModule.servlets.urls")
				|| pdName.equals("portletAppModule")
				|| pdName.equals("portletModule")) {
			return false;
		}
		return true;
	}
    ...
    public String getProxyClass() {
        return "com.ixora.rms.agents.websphere.v60.proxy.PmiClientProxyImpl";
    }
