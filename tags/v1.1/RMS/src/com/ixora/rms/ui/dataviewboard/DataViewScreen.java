...
public final class DataViewScreen extends JDesktopPane implements HTMLProvider {
	 ...
	 * Constructor.
	 */
	public DataViewScreen(String name) {
		super();
		setName(name);
	 ...
	 * @return
	 */
	public DataViewBoard getBoard(String title) {
		Collection<DataViewBoard> boards = getBoards();
		if(!Utils.isEmptyCollection(boards)) {
			for(DataViewBoard dataViewBoard : boards) {
				if(dataViewBoard.getTitle().equals(title)) {
					return dataViewBoard;
				}
			}
		}
		return null;
	 ...
	 * @return
	 */
	public List<DataViewBoard> getAvailableDataViewBoards(DataViewControl control) {
		List<DataViewBoard> ret = new LinkedList<DataViewBoard>();
		Collection<DataViewBoard> boards = getBoards();
		if(!Utils.isEmptyCollection(boards)) {
			for(DataViewBoard board : boards) {
				if(board != control.getOwner() && board.canPlotControl(control)){
					ret.add(board);
				}
			}
		}
