...
/**
 * WinPerfEntity
 * The actual object which fits into the agent framework, gets
 * created from a NativeObject or a NativeInstance, depending on
 * the level in hierarchy.
 */
public class WinPerfEntity extends Entity {
	 ...
	 * @throws Throwable
	 */
	public WinPerfEntity(
			String machine,
			EntityId idParent,
			NativeObject nativeObject,
			AgentExecutionContext ctxt) throws Throwable {
		// Create a main entity with the _Total instance
		super(new EntityId(idParent, nativeObject.name), nativeObject.description, ctxt);
	...
	/**
	 * Initializes counters for both object-level and instance-level entities. If
	 * nativeInstance is null then this is an object-level entity.
	 * @param nativeObject windows performance object
	 * @param nativeInstance instance of the above object; may be null
	 */
	private void updateCounters(NativeObject nativeObject, NativeInstance nativeInstance) {
		for (int i = 0; i < nativeObject.counters.length; i++) {
			NativeCounter c = nativeObject.counters[i];
			// don't show hidden counters
			if (c != null && (c.type & WinPerfConstants.PERF_DISPLAY_NOSHOW) != WinPerfConstants.PERF_DISPLAY_NOSHOW) {
				...
				if(this.getCounter(new CounterId(c.name)) == null) {
					String nativePath = WinPerfRootEntity.makeNativePath(
							this.machine,
							nativeObject.name,
							nativeInstance != null ? nativeInstance.name : nativeObject.totalInstanceName,
	...
	/**
	 * Called either when creating or updating an entity from a performance object
	 * @param nativeObject performance object to turn into an entity (or update existing)
	 * @throws Throwable
	 */
	public void updateObject(NativeObject nativeObject) throws Throwable {
		...
		if(!Utils.isEmptyArray(nativeObject.instances)) {
			...
			for(int j = 0; j < nativeObject.instances.length; j++) {
				...
				WinPerfEntity instanceEntity = (WinPerfEntity)fChildrenEntities.get(childId);
				if (instanceEntity == null) {
				    // Create a new one
					addChildEntity(new WinPerfEntity(fEntityId, nativeObject, ni, fContext));
				} else {
	 ...
	 * @param nativeObject parent object of this instance
	 * @param nativeInstance instance to create an entity from
	 */
	...
	protected void retrieveCounterValues() throws Throwable {
		...
		CounterValue[] data = WinPerfRootEntity.CollectQueryData(nativeQuery, nativeHandles, nativeFormats);
		if (data != null) {
	...
	protected void enableCounters(Set<CounterId> ec) throws Throwable {
		...
		nativeHandles = WinPerfRootEntity.SetCounters(machine, nativeQuery, counters);
	}
}