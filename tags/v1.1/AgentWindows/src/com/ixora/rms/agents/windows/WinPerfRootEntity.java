...
/**
 * WinPerfRootEntity
 * Main class responsible with retrieving object and instance data from
 * the native DLL and organizing it into two levels of entities: first
 * level contains objects and second contains instances. If an object
 * has a _Total instance, it is removed and it's counters are moved to
 * the object entity.
...
public class WinPerfRootEntity extends RootEntity {
	...
	protected static native CounterValue[] CollectQueryData(
		int nativeQuery, int[] nativeHandles, int[] formats)
		throws WinPerfException;

	protected static native int[] SetCounters(
		String machine, int nativeQuery, String[] counters)
		throws WinPerfException;

	protected static native int ResetQuery(
		int[] oldNativeHandles, int oldNativeQuery)
		throws WinPerfException;
	 ...
	 */
	public static String makeNativePath(String machine,
			String object, String instance, int instanceIndex, String counter) {
	 ...
	 * @param ctxt
	 */
	public WinPerfRootEntity(AgentExecutionContext ctxt) throws WinPerfNotSupportedException {
		super(ctxt);
	...
	public void updateChildrenEntities(boolean recursive) throws Throwable {
        ...
        try {
    		...
    		NativeObject[] objects = EnumEntities(machine, userName, domain, password);
    		for(int i = 0; i < objects.length; i++) {
    			NativeObject object = objects[i];
    			if(object != null) {
    				WinPerfEntity topLevel = (WinPerfEntity)fChildrenEntities.get(createEntityId(fEntityId, object));
    				if(topLevel == null) {
    				    // Create a new one
    					addChildEntity(new WinPerfEntity(this.machine, fEntityId, objects[i], fContext));
    				} else {
	 ...
	 * @param parent
	 * @param no
	 * @return an entity id that corresponds to the given native object and parent
	 */
