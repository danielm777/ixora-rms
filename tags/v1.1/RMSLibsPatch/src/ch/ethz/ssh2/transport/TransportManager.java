 ...
 * @version $Id: TransportManager.java,v 1.13 2006/02/14 19:43:15 cplattne Exp $
 */
// Some modifications for IxoraRMS
public class TransportManager
{
	...
	final Socket sock = new Socket(Proxy.NO_PROXY);
	// to allow for the reuse of the inet address
	...
	// ixora BEGIN
	public TransportManager(InetSocketAddress addr) throws IOException {
		this.inetAddr = addr;
	}
	// ixora END
			...
			int connectTimeout, SecureRandom rnd) throws IOException
	{
		...
		if(inetAddr == null) {
			...
			inetAddr = new InetSocketAddress(addr, port);
		}
		sock.connect(inetAddr, connectTimeout);
		// ixora - END
		sock.setSoTimeout(0);
